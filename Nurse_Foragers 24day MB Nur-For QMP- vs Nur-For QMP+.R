library("DESeq2")
setwd("D:/24.03.2020/Dokumente/Experiment/2019.04.09 Honeybee Cage experiment/Results/Result_geneid/Nures-Foragers 24day old/R analysis/Nurse_Foragers 24day MB")
sampleNames <- c("NuFA1MB_Control","NuFC2MB_Control","NuFC3MB_Control","NuFD1MB_Control","NuFD2MB_Control","NuFD3MB_Control",
                 "NuFA1MB_QMP","NuFA2MB_QMP","NuFC1MB_QMP","NuFC2MB_QMP","NuFD1MB_QMP","NuFD2MB_QMP")
mydata <- read.table("Nurse_Foragers 24day MB Nur-For QMP- vs Nur-For QMP+.txt", header = TRUE)
...
res <- results(dds)
write.table(res,"Nurse_Foragers 24day MB Nur-For QMP- vs Nur-For QMP+ result.csv", sep = ",", row.names = TRUE)
head(res)
summary(res)
rld <- rlog(dds)
