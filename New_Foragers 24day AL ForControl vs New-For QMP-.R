library("DESeq2")
setwd("E:/24.03.2020/Dokumente/Experiment/2019.04.09 Honeybee Cage experiment/Results/Result_geneid/New-Foragers 24day old/R analysis/New_Foragers 24day AL")
...
mydata <- read.table("New_Foragers 24day AL ForControl vs New-For QMP-.txt", header = TRUE)
...
res <- results(dds)
write.table(res,"New_Foragers 24day AL ForControl vs New-For QMP- result.csv", sep = ",", row.names = TRUE)
head(res)
summary(res)
rld <- rlog(dds)
...
p<-ggplot(data1,aes(PC1,PC2,color=condition, shape=colony))+
  scale_color_manual(values = c("#7fbf7b","#fc8d62"))+
  geom_point(size=8)+
