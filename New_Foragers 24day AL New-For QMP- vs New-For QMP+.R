library("DESeq2")
setwd("D:/24.03.2020/Dokumente/Experiment/2019.04.09 Honeybee Cage experiment/Results/Result_geneid/New-Foragers 24day old/R analysis/New_Foragers 24day AL")
sampleNames <- c("NeFA1AL_Control","NeFA2AL_Control","NeFC1AL_Control","NeFC2AL_Control","NeFD1AL_Control","NeFD3AL_Control",
                 "NeFA1AL_QMP","NeFC1AL_QMP","NeFC2AL_QMP","NeFD1AL_QMP","NeFD2AL_QMP","NeFD3AL_QMP")
mydata <- read.table("New_Foragers 24day AL New-For QMP- vs New-For QMP+.txt", header = TRUE)
...
res <- results(dds)
write.table(res,"New_Foragers 24day AL New-For QMP- vs New-For QMP+ result.csv", sep = ",", row.names = TRUE)
head(res)
summary(res)
rld <- rlog(dds)
