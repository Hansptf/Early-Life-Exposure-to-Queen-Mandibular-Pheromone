...
sampleNames <- c("ForA1AT_Control","ForA2AT_Control","ForB1AT_Control","ForB2AT_Control","ForC1AT_Control","ForC2AT_Control",
                 "ForA2AT_QMP","ForB1AT_QMP","ForB4AT_QMP","ForC1AT_QMP","ForC2AT_QMP","ForC3AT_QMP")
mydata <- read.table("Foragers 21day AT.txt", header = TRUE)
...
table2 <- data.frame(name = c("Col1","Col2","Col3","Col4","Col5","Col6","QMP1","QMP2","QMP3","QMP4","QMP5","QMP6"),
                     ...
                     colony = c("1","1","2","2","3","3","1","2","2","3","3","3"))
...
res <- results(dds)
write.table(res,"Foragers 21day ATresult.csv", sep = ",", row.names = TRUE)
head(res)
summary(res)
rld <- rlog(dds)
