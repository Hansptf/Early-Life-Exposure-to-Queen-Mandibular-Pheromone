library("DESeq2")
setwd("E:/24.03.2020/Dokumente/Experiment/2019.04.09 Honeybee Cage experiment/Results/Result_geneid/Nures-Foragers 24day old/R analysis/Nurse_Foragers 24day AT")
sampleNames <- c("NuFC1AT_Control","NuFC2AT_Control","NuFD2AT_Control","NuFD3AT_Control","NuFD4AT_Control","NuFD5AT_Control",
                 "NuFA1AT_QMP","NuFA2AT_QMP","NuFC1AT_QMP","NuFC2AT_QMP","NuFD1AT_QMP","NuFD2AT_QMP")

mydata <- read.table("Nurse_Foragers 24day AT Nur-For QMP- vs Nur-For QMP+.txt", header = TRUE)
...
res <- results(dds)
write.table(res,"Nurse_Foragers 24day AT Nur-For QMP- vs Nur-For QMP+ result.csv", sep = ",", row.names = TRUE)
head(res)
summary(res)
rld <- rlog(dds)
