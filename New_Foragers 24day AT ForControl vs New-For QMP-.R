...
mydata <- read.table("New_Foragers 24day AT ForControl vs New-For QMP-.txt", header = TRUE)
...
res <- results(dds)
write.table(res,"New_Foragers 24day AT ForControl vs New-For QMP- result.csv", sep = ",", row.names = TRUE)
head(res)
summary(res)
rld <- rlog(dds)
