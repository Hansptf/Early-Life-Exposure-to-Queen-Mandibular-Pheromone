library("DESeq2")
setwd("E:/24.03.2020/Dokumente/Experiment/2019.04.09 Honeybee Cage experiment/Results/Result_geneid/New-Foragers 24day old/R analysis/New_Foragers 24day AT")
sampleNames <- c("NeFA1AT_Control","NeFA2AT_Control","NeFC1AT_Control","NeFC2AT_Control","NeFD1AT_Control","NeFD3AT_Control",
                 "NeFA1AT_QMP","NeFA2AT_QMP","NeFC1AT_QMP","NeFC2AT_QMP","NeFD2AT_QMP","NeFD3AT_QMP")
mydata <- read.table("New_Foragers 24day AT New-For QMP- vs New-For QMP+.txt", header = TRUE)
names(mydata)[2:13] <- sampleNames
countMatrix <- as.matrix(mydata[2:13])
rownames(countMatrix) <-mydata$Geneid
table2 <- data.frame(name = c("ColNew1","ColNew2","ColNew3","ColNew4","ColNew5","ColNew6",
                              "QMP1","QMP2","QMP3","QMP4","QMP5","QMP6"),
                     condition = c("ColNew","ColNew","ColNew","ColNew","ColNew","ColNew",
...
res <- results(dds)
write.table(res,"New_Foragers 24day AT New-For QMP- vs New-For QMP+ result.csv", sep = ",", row.names = TRUE)
head(res)
summary(res)
rld <- rlog(dds)
